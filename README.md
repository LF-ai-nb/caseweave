...
- 提供可运行示例、单元测试、CI 配置和 Mooncakes 包发布配置。
...
这让项目从“生成第一版矩阵”延展到“验证遗留测试并生成补测计划”，更适合接入真实 CI。

## 独特性说明

项目已按 Mooncakes/GitHub 上常见测试类方向做过对比：CaseWeave 不做随机性质测试、不做代码覆盖率报告、不做 TAP 报告、不做提交材料自查，也不是通用规则/novelty 分析工具。它的边界是“确定性、带约束、可验证、可导出的组合测试矩阵”，新增的场景规格与风险门禁进一步把它定位为发布前测试计划生成器。

CaseWeave 是原创 MoonBit 开源库，不是已有 Mooncakes 审查、README/许可证校验、来源证明、机器人策略或报名材料审核工具的移植、改名或扩展版本。仓库里的 `audit` 指“测试矩阵覆盖验证”：它只判断给定测试用例是否覆盖目标参数交互，不读取其他仓库，不审查包发布资格，也不做开源来源溯源。

更完整的查重和边界说明见 `docs/DIFFERENTIATION.md`。

## 项目边界

CaseWeave 负责生成和验证测试矩阵，不负责执行测试、调用浏览器、管理环境或写入文件。这样的边界让它适合作为库被 CI、命令行工具或业务测试框架调用。
...
- `cmd/main`：可运行示例
- `docs/`：API、设计、差异化说明、验收和验证记录

## Mooncakes 发布

当前包名为 `LF-ai-nb/caseweave`，仓库为 `https://github.com/LF-ai-nb/caseweave`，已按该身份发布到 Mooncakes。后续版本发布前请继续确认 `moon.mod` 里的包名和仓库地址对应 Mooncakes / GitHub 账号。
